...
### Channel A: YouTube Tutorial Channel 📺
...
1. At the top of the screen, tap any mode: FOCUS • STUDY • CHILL • FLOW • DEEP
...
2. **Automatic caption extraction**: The app uses a bulletproof 3-method fallback system:
   - ✅ **Method 0**: Cache check (instant if previously loaded)
   - ✅ **Method 1**: youtube-caption-extractor (fast, works on most videos)
   - ✅ **Method 2**: youtube-transcript package (reliable fallback)
   - ⏱️ **15-second timeout per method** prevents infinite loading (30 seconds max total)
   - ✅ **Smart caching**: First download caches transcript for 7 days - repeat visits load instantly
   - ✅ **Offline access**: View cached transcripts even without internet
   - ✅ **Swipe to dismiss**: Swipe down anywhere on the transcript screen to close and return to video
   - ❌ **If no captions**: You'll see a helpful error screen explaining why and what to do
...
│   │   ├── transcript-ai.ts        # AI-powered transcript analysis (including Lesson Breakdown)
│   │   └── chat-service.ts         # OpenAI & Grok API integration
│   ├── state/
│   │   ├── appStore.ts             # Zustand store with audio modes & notes
